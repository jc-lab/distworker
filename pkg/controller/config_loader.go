...
func setDefaults(config *Config) {
	...
	for name, settings := range config.Provisioner {
		if settings.Worker.IdleTimeout == 0 {
			settings.Worker.IdleTimeout = 1 * time.Minute
		}
		if settings.Worker.MaxTaskDuration == 0 {
			settings.Worker.MaxTaskDuration = 1 * time.Hour
		}
		if settings.Worker.Max == 0 {
			settings.Worker.Max = 5
		}
...
func validateConfig(config *Config) error {
	...
	for name, settings := range config.Provisioner {
		...
		if settings.Worker.Max <= 0 {
			return fmt.Errorf("provisioner.%s.worker.max must be greater than 0", name)
		}
		if settings.Worker.Min < 0 {
			return fmt.Errorf("provisioner.%s.worker.min must be greater than or equal to 0", name)
		}
		if settings.Worker.Min > settings.Worker.Max {
			return fmt.Errorf("provisioner.%s.worker.min must be less than or equal to max", name)
		}
	}
